...
while (j<min(s,len(limit_sets))):
	...
	filtered_tokens = [x for x in all_tokens if x not in limit_set]
...
for j in range(0,len(limit_sets)):	
	if (len(limit_sets[j][0])==3):
		if (limit_sets[j][0][0]!=limit_sets[j-1][0][0]):
			L.append('ideas.'+limit_sets[j][0][0]+',');
		if (limit_sets[j][0][1]!=limit_sets[j-1][0][1]):
			L.append('ideas.'+limit_sets[j][0][0]+'.'+limit_sets[j][0][1]+',');
		L.append('ideas.'+limit_sets[j][0][0]+'.'+limit_sets[j][0][1]+'.'+limit_sets[j][0][2]+',');
