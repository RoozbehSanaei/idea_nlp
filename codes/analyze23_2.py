...
af = cluster.KMeans(n_clusters=20).fit(cl_mat)
clusters = af.labels_
print(clusters)

""
from sklearn import cluster
bandwidth = cluster.estimate_bandwidth(cl_mat, quantile=0.3, n_samples=500)
af = cluster.MeanShift().fit(cl_mat)
clusters = af.labels_
print(clusters)
...
from sklearn import cluster
af = cluster.AgglomerativeClustering().fit(cl_mat)
clusters = af.labels_
print(clusters)
