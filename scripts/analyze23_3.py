...
'n1400 distributed novice',
'n1400 group experienced',
'n100 group novice',
...
bc_sents = bc['tokens'].apply(lambda x: getLemmas(x))

rma['tokens'] = rma.ix[:,2].progress_apply(lambda x: cleanPassage(x))    
rma_sents = rma['tokens'].apply(lambda x: getLemmas(x))

sentences = list(bc_sents)

bc_cats = bc.ix[:,1]
rma_cats = rma.ix[:,0]

bc_cats_set = list(set(bc_cats))
rma_cats_set = list(set(rma_cats))


bc_mat = numpy.zeros((len(bc_sents),len(bc_sents)))
rma_mat = numpy.zeros((len(rma_sents),len(rma_sents)))
cl_mat = numpy.zeros((len(bc_sents),len(bc_sents)))


for sent1 in bc_sents:
    for sent2 in bc_sents:
        i1 = sentences.index(sent1)
        i2 = sentences.index(sent2)
        if (bc_cats[i1]==bc_cats[i2]):
            bc_mat[i1][i2] = 1


for sent1 in rma_sents:
    for sent2 in rma_sents:
        i1 = sentences.index(sent1)
        i2 = sentences.index(sent2)
        if (rma_cats[i1]==rma_cats[i2]):
            rma_mat[i1][i2] = 1
...
for sent1 in sentences:
    for sent2  in sentences:
        i1 = sentences.index(sent1)
        i2 = sentences.index(sent2)
        cl_mat[i1][i2] = max_set_similairy(sent1,sent2)


from sklearn import cluster
af = cluster.AffinityPropagation(preference=-3).fit(cl_mat)
clusters = af.labels_
print(clusters)
...
print("number of sentence couples brad clustered together : ",end = "")
print(numpy.count_nonzero(bc_mat))
print("number of sentence couples ryan clustered together : ",end = "")
print(numpy.count_nonzero(rma_mat))
print("number of sentence couples computer clustered together : ",end = "")
print(numpy.count_nonzero(cl_mat))
print ("")

print("number of sentence couples ryan and brad clustered the same way : ",end = "")
print(numpy.count_nonzero(bc_mat==rma_mat))
print("number of sentence couples ryan and brad did not clustered  the same way : ",end = "")
print(numpy.count_nonzero(bc_mat!=rma_mat))
print("")

print("number of word couples computer and brad clustered in the same way : ",end = "")
print(numpy.count_nonzero(bc_mat==cl_mat))
print("number of word couples computer and brad did not clustered in the same way : ",end = "")
print(numpy.count_nonzero(bc_mat!=cl_mat))
print("")

print("number of word couples computer and ryan clustered in the same way : ",end = "")
print(numpy.count_nonzero(rma_mat==cl_mat))
print("number of word couples computer and ryan did not clustered in the same way : ",end = "")
print(numpy.count_nonzero(rma_mat!=cl_mat))
print("")


print("ryan vs brad ")
print( numpy.linalg.norm(numpy.dot(bc_mat,rma_mat))/numpy.linalg.norm(bc_mat)*numpy.linalg.norm(rma_mat))
print("brad vs computer ")
print( numpy.linalg.norm(numpy.dot(cl_mat,bc_mat))/numpy.linalg.norm(bc_mat)*numpy.linalg.norm(cl_mat))
print("ryan vs computer ")
print( numpy.linalg.norm(numpy.dot(cl_mat,rma_mat))/numpy.linalg.norm(cl_mat)*numpy.linalg.norm(rma_mat))
